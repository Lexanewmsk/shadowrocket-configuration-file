...
if ($request) {
    ...
    if (randomUA.includes("Chrome")) {
        ...
        headers["DNT"] = "1"; // Do Not Track для Firefox
    }
    ...
    headers["Referer"] = "https://www.google.com/"; // Маскировка реферала
    ...
    console.log(`Processed URL: ${url}, User-Agent: ${headers["User-Agent"]}, Sec-Ch-Ua-Platform: ${headers["Sec-Ch-Ua-Platform"] || "N/A"}`);
...
if ($response) {
    ...
    if (body) {
        // Подмена платформы и других данных
        ...
        body = body.replace(/navigator\.platform\s*=\s*['"]MacIntel['"]/g, 'navigator.platform = "Win32"');
        ...
        body = body.replace(/navigator\.hardwareConcurrency\s*=\s*\d+/g, 'navigator.hardwareConcurrency = 8');
    }
    $done({body});
}
